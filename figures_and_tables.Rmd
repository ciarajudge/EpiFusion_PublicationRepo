...
ebola_fit <- fittable(ebola, "Combined", "EBOV_Sierra") %>%
  mutate(Date = as.Date("2014-03-20", format = "%Y-%m-%d") + Time) 

key_epidemic_events <- as.Date(c("2014-08-06", "2014-09-19", "2014-10-01"))
...
ebola_rt <- ggplot(ebola_fit, aes(x = Date)) +
  geom_hline(aes(yintercept = 1), linetype = 2, color = "darkgrey", linewidth = 0.5) +
  labs(y = "Effective Reproduction Number") +
  coord_cartesian(xlim = as.Date(c("2014-03-20","2015-08-04"), format = "%Y-%m-%d"))+
  geom_line(aes(y = Mean_Rt), col = "#2aac6d") +
  geom_ribbon(aes(ymin = Lower95_Rt, ymax = Upper95_Rt), fill = "#2aac6d", alpha = 0.2) +
  geom_ribbon(aes(ymin = Lower88_Rt, ymax = Upper88_Rt), fill = "#2aac6d", alpha = 0.2) +
  geom_ribbon(aes(ymin = Lower66_Rt, ymax = Upper66_Rt), fill = "#2aac6d", alpha = 0.2) +
  lshtm_theme() +
...
fig9 <- ggarrange(ebola_tree_plot, ebola_incidence_plot, ebola_rt, ncol = 1, align = "v", heights = c(2.8,1.4,1.6), labels = c("A", "B", "C"), font.label = list(color = "#01454f", family = NULL))
ggsave("Figs/Figure9.pdf", fig9, width = 8, height = 10.5, units = "in")
```
...
figS5 <- ggarrange(figS5a, figS5b, nrow = 1, labels = c("A", "B"), font.label = list(color = "#01454f", family = NULL))

figS5

ggsave("Figs/FigureS5.pdf", figS5, width = 7, height = 3.5, units = "in")
...
tableS1 <- data.frame(Characteristic = colnames(tableS1), Mean = colMedians(as.matrix(tableS1)))
